...
{
    public class WorkFlowTask : IEqualityComparer<MyCreek.Data.Model.WorkFlowTask>
    {
        private WorkFlow bWorkFlow = new WorkFlow();
        private MyCreek.Data.Interface.IWorkFlowTask dataWorkFlowTask;
        private MyCreek.Data.Model.WorkFlowInstalled wfInstalled;
        private MyCreek.Data.Model.WorkFlowExecute.Result result;
        private List<MyCreek.Data.Model.WorkFlowTask> nextTasks = new List<MyCreek.Data.Model.WorkFlowTask>();
		public WorkFlowTask()
		{
        ...
        /// <returns></returns>
        public bool Equals(MyCreek.Data.Model.WorkFlowTask task1, MyCreek.Data.Model.WorkFlowTask task2)
        {
            return task1.ReceiveID == task2.ReceiveID;
        }

        public int GetHashCode(MyCreek.Data.Model.WorkFlowTask task)
        {
        ...
        public string GetStepSnderIDString(Guid flowID, Guid stepID, Guid groupID)
        {
            ...
            {
                sb.Append(MyCreek.Platform.Users.PREFIX);
                sb.Append(li);
                sb.Append(",");
            }
        ...
        /// <returns></returns>
        private MyCreek.Data.Model.WorkFlowExecute.Execute GetExecuteModel(string jsonString)
        {
            MyCreek.Data.Model.WorkFlowExecute.Execute execute = new MyCreek.Data.Model.WorkFlowExecute.Execute();
            MyCreek.Platform.Organize borganize = new Organize();
            ...
            { 
                case "submit":
                    execute.ExecuteType = MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Submit;
                    break;
                case "save":
                    execute.ExecuteType = MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Save;
                    break;
                case "back":
                    execute.ExecuteType = MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back;
                    break;
            }
            ...
            Dictionary<Guid, List<MyCreek.Data.Model.Users>> steps = new Dictionary<Guid, List<MyCreek.Data.Model.Users>>();
        ...
        /// <param name="jsonString"></param>
        /// <returns></returns>
        public MyCreek.Data.Model.WorkFlowExecute.Result Execute(string jsonString)
        {
        ...
        /// <returns></returns>
        public MyCreek.Data.Model.WorkFlowExecute.Result Execute(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel)
        {
            result = new MyCreek.Data.Model.WorkFlowExecute.Result();
            nextTasks = new List<MyCreek.Data.Model.WorkFlowTask>();
            ...
            lock (executeModel.GroupID.ToString())
            {
                ...
                {
                    case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back:
                        executeBack(executeModel);
                        break;
                    //case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Completed:
                    //    executeComplete(executeModel);
                    //    break;
                    case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Save:
                        executeSave(executeModel);
                        break;
                    case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Submit:
                    case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Completed:
                        executeSubmit(executeModel);
                        break;
                    case MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Redirect:
        ...
        private void executeSubmit(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel)
        {
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
            {
                //如果是第一步提交并且没有实例则先创建实例
                MyCreek.Data.Model.WorkFlowTask currentTask = null;
                ...
                foreach (var step in executeModel.Steps)
                {
                    foreach (var user in step.Value)
                    {
                        ...
                        if (isPassing)
                        {
                            MyCreek.Data.Model.WorkFlowTask task = new MyCreek.Data.Model.WorkFlowTask();
                            if (nextStep.WorkTime > 0)
                            {
                                task.CompletedTime = MyCreek.Utility.DateTimeNew.Now.AddHours((double)nextStep.WorkTime);
                            }
                            ...
                            task.ReceiveTime = MyCreek.Utility.DateTimeNew.Now;
                            ...
                            {
                                MyCreek.Data.Model.WorkFlowExecute.Execute subflowExecuteModel = new MyCreek.Data.Model.WorkFlowExecute.Execute();
                                if (!nextStep.Event.SubFlowActivationBefore.IsNullOrEmpty())
                                {
                                    object obj = ExecuteFlowCustomEvent(nextStep.Event.SubFlowActivationBefore.Trim(),
                                        new MyCreek.Data.Model.WorkFlowCustomEventParams()
                                        {
                                    ...
                                    if (obj is MyCreek.Data.Model.WorkFlowExecute.Execute)
                                    {
                                        subflowExecuteModel = obj as MyCreek.Data.Model.WorkFlowExecute.Execute;
                                    }
                                }
                                subflowExecuteModel.ExecuteType = MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Save;
        ...
        private void executeSave(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel)
        {
            //如果是第一步提交并且没有实例则先创建实例
            MyCreek.Data.Model.WorkFlowTask currentTask = null;
            bool isFirst = executeModel.StepID == wfInstalled.FirstStepID && executeModel.TaskID == Guid.Empty && executeModel.GroupID == Guid.Empty;
        ...
        private void executeBack(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel)
        {
            ...
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
            {
                List<MyCreek.Data.Model.WorkFlowTask> backTasks = new List<MyCreek.Data.Model.WorkFlowTask>();
                int status = 0;
                ...
                List<MyCreek.Data.Model.WorkFlowTask> tasks = new List<MyCreek.Data.Model.WorkFlowTask>();
                foreach (var step in executeModel.Steps)
                {
                ...
                if (isBack)
                {
                    foreach (var task in tasks.Distinct(this))
                    {
                        if (task != null)
                        {
                            MyCreek.Data.Model.WorkFlowTask newTask = task;
                            newTask.ID = Guid.NewGuid();
                            newTask.PrevID = currentTask.ID;
                            newTask.Note = "退回任务";
                            newTask.ReceiveTime = MyCreek.Utility.DateTimeNew.Now;
                            newTask.SenderID = currentTask.ReceiveID;
                            newTask.SenderName = currentTask.ReceiveName;
                            newTask.SenderTime = MyCreek.Utility.DateTimeNew.Now;
                            ...
                            {
                                newTask.CompletedTime = MyCreek.Utility.DateTimeNew.Now.AddHours((double)currentStep.WorkTime);
                            }
                            else
                            {
        ...
        private void executeComplete(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel, bool isCompleteTask = true)
        {
            ...
            if (parentTasks.Count > 0)
            {
                ...
                if (flowRunModel != null)
                {
                    ...
                    {
                        ExecuteFlowCustomEvent(steps.First().Event.SubFlowCompletedBefore.Trim(), new MyCreek.Data.Model.WorkFlowCustomEventParams()
                        {
        ...
        private void executeRedirect(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel)
        {
            MyCreek.Data.Model.WorkFlowTask currentTask = null;
            bool isFirst = executeModel.StepID == wfInstalled.FirstStepID && executeModel.TaskID == Guid.Empty && executeModel.GroupID == Guid.Empty;
            if (isFirst)
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        private bool IsPassing(MyCreek.Data.Model.WorkFlowInstalledSub.Step step, Guid flowID, Guid groupID, Guid taskID)
        {
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        private bool IsBack(MyCreek.Data.Model.WorkFlowInstalledSub.Step step, Guid flowID, Guid groupID, Guid taskID)
        {
            var tasks = GetTaskList(flowID, step.ID, groupID).FindAll(p => p.PrevID == taskID);
            if (tasks.Count == 0)
            {
        ...
        /// <returns></returns>
        private MyCreek.Data.Model.WorkFlowTask createFirstTask(MyCreek.Data.Model.WorkFlowExecute.Execute executeModel, bool isSubFlow = false)
        {
            ...
            {
                task.CompletedTime = MyCreek.Utility.DateTimeNew.Now.AddHours((double)nextSteps.First().WorkTime);
            }
            task.FlowID = executeModel.FlowID;
        ...
        {
            return dataWorkFlowTask.GetTasks(userID, out pager, query, title, flowid, MyCreek.Platform.Users.RemovePrefix(sender), date1, date2, type);
        }
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetTaskList(Guid flowID, Guid stepID, Guid groupID)
        {
        ...
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetTaskListByStepID(Guid taskID, Guid stepID)
        {
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetPrevTaskList(Guid taskID)
        {
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetNextTaskList(Guid taskID)
        {
        ...
        /// <param name="stepID"></param>
        /// <returns></returns>
        public Dictionary<Guid, string> GetBackSteps(Guid taskID, int backType, Guid stepID, MyCreek.Data.Model.WorkFlowInstalled wfInstalled)
        {
        ...
        /// <param name="userID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetUserTaskList(Guid flowID, Guid stepID, Guid groupID, Guid userID)
        {
        ...
        /// <returns></returns>
        public string DesignateTask(Guid taskID, MyCreek.Data.Model.Users user)
        {
            var task = Get(taskID);
            if (task == null)
            {
        ...
        public string BackTask(Guid taskID)
        {
            ...
            MyCreek.Data.Model.WorkFlowExecute.Execute executeModel = new MyCreek.Data.Model.WorkFlowExecute.Execute();
            executeModel.ExecuteType = MyCreek.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back;
            ...
            Dictionary<Guid, List<MyCreek.Data.Model.Users>> execSteps = new Dictionary<Guid, List<MyCreek.Data.Model.Users>>();
            var backsteps = GetBackSteps(taskID, steps.First().Behavior.BackType, task.StepID, wfInstalled);
            foreach (var back in backsteps)
            {
                execSteps.Add(back.Key, new List<MyCreek.Data.Model.Users>());
            }
        ...
        public List<MyCreek.Data.Model.WorkFlowTask> Sort(List<MyCreek.Data.Model.WorkFlowTask> tasks)
        {
        ...
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowTask> GetBySubFlowGroupID(Guid subflowGroupID)
        {
