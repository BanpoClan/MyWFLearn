...
namespace MyCreek.Platform
{
    ...
    {
        private MyCreek.Data.Interface.IWorkFlow dataWorkFlow;
        public WorkFlow()
        {
        ...
        public string SaveFlow(string jsonString)
        {
            ...
            else
            {
                ...
                MyCreek.Data.Model.WorkFlow wf = bwf.Get(flowID);
                bool isAdd = false;
                if (wf == null)
                {
                    wf = new MyCreek.Data.Model.WorkFlow();
                    isAdd = true;
                    wf.ID = flowID;
                    wf.CreateDate = MyCreek.Utility.DateTimeNew.Now;
                    wf.CreateUserID = MyCreek.Platform.Users.CurrentUserID;
                    wf.Status = 1;
                }
        ...
        public string InstallFlow(string jsonString, bool isMvc = true)
        {
            ...
            MyCreek.Data.Model.WorkFlowInstalled wfInstalled = GetWorkFlowRunModel(jsonString, out errMsg);
            if (wfInstalled == null)
            {
                return errMsg;
            }
            ...
            {
                MyCreek.Data.Model.WorkFlow wf = dataWorkFlow.Get(wfInstalled.ID);
                if (wf == null)
                {
                    return "流程实体为空";
                }
                else
                {
                    using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        #region 添加到应用程序库
                        MyCreek.Platform.AppLibrary bappLibrary = new AppLibrary();
                        MyCreek.Data.Model.AppLibrary app = bappLibrary.GetByCode(wfInstalled.ID.ToString());
                        bool isAdd = false;
                        if (app == null)
                        {
                            isAdd = true;
                        ...
                        #endregion
                        MyCreek.Cache.IO.Opation.Set(getCacheKey(wfInstalled.ID), wfInstalled);
                        scope.Complete();
                        return "1";
                    }
                }
        ...
        public MyCreek.Data.Model.WorkFlow SaveAs(Guid flowID, string newName)
        {
            MyCreek.Data.Model.WorkFlow wf = dataWorkFlow.Get(flowID);
        ...
        {
            return string.Concat(MyCreek.Utility.Keys.CacheKeys.WorkFlowInstalled_.ToString(), flowID.ToString("N"));
        }
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public MyCreek.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(string flowID, bool cache = true)
        {
            Guid fid;
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public MyCreek.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(Guid flowID, bool cache = true)
        {
            if (!cache)
            {
            ...
            else
            {
                ...
                if (obj == null)
                {
                    ...
                    MyCreek.Cache.IO.Opation.Set(key, wfi);
                    return wfi;
                }
                else
                {
                    return obj as MyCreek.Data.Model.WorkFlowInstalled;
                }
            }
  
        }

        private MyCreek.Data.Model.WorkFlowInstalled getWorkFlowRunFromDesign(Guid flowID)
        {
            var wf = Get(flowID);
        ...
        {
            string key = getCacheKey(flowID);
            MyCreek.Cache.IO.Opation.Set(key, GetWorkFlowRunModel(flowID, false));
        }
        ...
        public MyCreek.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(string jsonString, out string errMsg)
        {
            errMsg = "";
            MyCreek.Data.Model.WorkFlowInstalled wfInstalled = new MyCreek.Data.Model.WorkFlowInstalled();
            ...
            wfInstalled.DebugUsers = new MyCreek.Platform.Organize().GetAllUsers(json["debugUsers"].ToString());
            wfInstalled.Note = json["note"].ToString();

            List<MyCreek.Data.Model.WorkFlowInstalledSub.DataBases> dataBases = new List<MyCreek.Data.Model.WorkFlowInstalledSub.DataBases>();
            var dbs = json["databases"];
            if (dbs.IsArray)
            {
                foreach (LitJson.JsonData db in dbs)
                {
                    dataBases.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.DataBases()
                    {
            ...
            {
                wfInstalled.TitleField = new MyCreek.Data.Model.WorkFlowInstalledSub.TitleField()
                {
            ...
            #region 载入步骤信息
            List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> stepsList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.Step>();
            LitJson.JsonData steps = json["steps"];
            if (steps.IsArray)
            {
                foreach (LitJson.JsonData step in steps)
                {
                    ...
                    MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Behavior behavior1 = new MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Behavior();
                    ...
                    List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Button> buttionList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Button>();
                    if (buttons.IsArray)
                    {
                        foreach (LitJson.JsonData button in buttons)
                        {
                            ...
                            buttionList.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Button()
                            {
                                ID = butID,
                    ...
                    MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Event event2 = new MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Event();
                    if (event1.IsObject)
                    {
                    ...
                    List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Form> formList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Form>();
                    if (forms.IsArray)
                    {
                        foreach (LitJson.JsonData form in forms)
                        {
                            formList.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.Form()
                            {
                    ...
                    List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus> fieldStatusList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus>();
                    if (fieldStatus.IsArray)
                    {
                        foreach (LitJson.JsonData field in fieldStatus)
                        {
                            fieldStatusList.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus()
                            {
                    ...
                    stepsList.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.Step()
                    {
            ...
            List<MyCreek.Data.Model.WorkFlowInstalledSub.Line> linesList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.Line>();
            LitJson.JsonData lines = json.ContainsKey("lines") ? json["lines"] : new LitJson.JsonData();
            if (lines.IsArray)
            {
                foreach (LitJson.JsonData line in lines)
                {
                    linesList.Add(new MyCreek.Data.Model.WorkFlowInstalledSub.Line()
                    {
            ...
            if (wf != null)
            {
                ...
                wfInstalled.InstallTime = MyCreek.Utility.DateTimeNew.Now;
        ...
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> GetAllPrevSteps(Guid flowID, Guid stepID)
        {
            List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.Step>();
        ...
        private void addPrevSteps(List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> list, MyCreek.Data.Model.WorkFlowInstalled wfInstalled, Guid stepID)
        {
        ...
        /// <param name="stepID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> GetPrevSteps(Guid flowID, Guid stepID)
        {
            List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.Step>();
            var wfInstalled = GetWorkFlowRunModel(flowID);
            if (wfInstalled == null)
        ...
        /// <param name="stepID"></param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> GetNextSteps(Guid flowID, Guid stepID)
        {
            List<MyCreek.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<MyCreek.Data.Model.WorkFlowInstalledSub.Step>();
            var wfInstalled = GetWorkFlowRunModel(flowID);
            if (wfInstalled == null)
        ...
        /// <param name="defautFirstStepName">如果步骤ID为空是否默认为第一步</param>
        /// <returns></returns>
        public string GetStepName(Guid stepID, MyCreek.Data.Model.WorkFlowInstalled wfinstalled, bool defautFirstStepName = false)
        {
            if (wfinstalled == null) return "";
        ...
        /// 保存表单数据
        /// </summary>
        public string SaveFromData(string instanceid, MyCreek.Data.Model.WorkFlowCustomEventParams eventParams)
        {
            //保存自定义表单内容
            ...
            MyCreek.Data.Model.DBConnection dbconn = bdbconn.Get(dbconnid.ToGuid());
            if (dbconn == null)
            {
                return instanceid;
            }
        ...
        public LitJson.JsonData GetFormData(string connid, string table, string pk, string instanceid, string filedStatus = "")
        {
            ...
            MyCreek.Platform.DBConnection bdbconn = new MyCreek.Platform.DBConnection();
            MyCreek.Data.Model.DBConnection dbconn = bdbconn.Get(connid.ToGuid());
            if (dbconn == null)
            {
                return "";
            }
