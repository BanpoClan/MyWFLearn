...
namespace MyCreek.Platform
{
    ...
    {
        private MyCreek.Data.Interface.IWorkFlowDelegation dataWorkFlowDelegation;
        private static string cacheKey = MyCreek.Utility.Keys.CacheKeys.WorkFlowDelegation.ToString();
        ...
        /// </summary>
        /// <returns></returns>
        public List<MyCreek.Data.Model.WorkFlowDelegation> GetNoExpiredListFromCache()
        {
            object obj = MyCreek.Cache.IO.Opation.Get(cacheKey);
            if (obj != null && obj is List<MyCreek.Data.Model.WorkFlowDelegation>)
            {
                return obj as List<MyCreek.Data.Model.WorkFlowDelegation>;
            }
            else
            {
                var list = GetNoExpiredList();
        ...
        public Guid GetFlowDelegationByUserID(Guid flowID, Guid userID)
        {
            ...
            var userList = list.Where(p => p.UserID == userID && (!p.FlowID.HasValue || p.FlowID.Value == Guid.Empty || p.FlowID.Value == flowID) && p.EndTime >= MyCreek.Utility.DateTimeNew.Now);
        ...
        private Guid getFlowDelegationByUserID1(Guid flowID, Guid userID, List<MyCreek.Data.Model.WorkFlowDelegation> list)
        {
