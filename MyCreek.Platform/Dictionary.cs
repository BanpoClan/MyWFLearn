...
namespace MyCreek.Platform
{
    ...
    {
        private MyCreek.Data.Interface.IDictionary dataDictionary;
        private static string cacheKey = MyCreek.Utility.Keys.CacheKeys.Dictionary.ToString();
        public Dictionary()
        {
        ...
        /// 查询所有记录
        /// </summary>
        public List<MyCreek.Data.Model.Dictionary> GetAll(bool fromCache=false)
        {
            if (!fromCache)
            {
            ...
            {
                object obj = MyCreek.Cache.IO.Opation.Get(cacheKey);
                if (obj != null && obj is List<MyCreek.Data.Model.Dictionary>)
                {
                    return obj as List<MyCreek.Data.Model.Dictionary>;
                }
                else
                {
                    var list = dataDictionary.GetAll();
                    MyCreek.Cache.IO.Opation.Set(cacheKey, list);
                    return list;
                }
            }
        }
        ...
        /// 查询单条记录
        /// </summary>
        public MyCreek.Data.Model.Dictionary Get(Guid id, bool fromCache = false)
        {
        ...
        /// 查询下级记录
        /// </summary>
        public List<MyCreek.Data.Model.Dictionary> GetChilds(Guid id, bool fromCache=false)
        {
        ...
        /// 查询下级记录
        /// </summary>
        public List<MyCreek.Data.Model.Dictionary> GetChilds(string code, bool fromCache=false)
        {
            return code.IsNullOrEmpty() ? new List<MyCreek.Data.Model.Dictionary>() :
                fromCache ? getChildsByCodeFromCache(code) :
                dataDictionary.GetChilds(code.Trim());
        }

        private List<MyCreek.Data.Model.Dictionary> getChildsByCodeFromCache(string code)
        {
            var list = GetAll(true);
            var dict = list.Find(p => string.Compare(p.Code, code, true) == 0);
            return dict == null ? new List<MyCreek.Data.Model.Dictionary>() : list.FindAll(p => p.ParentID == dict.ID).OrderBy(p=>p.Sort).ToList();
        }

        private List<MyCreek.Data.Model.Dictionary> getChildsByIDFromCache(Guid id)
        {
            var list = GetAll(true);
        ...
        /// <returns></returns>
        public List<MyCreek.Data.Model.Dictionary> GetAllChilds(string code, bool fromCache)
        {
            if (code.IsNullOrEmpty()) return new List<MyCreek.Data.Model.Dictionary>();
            var dict = GetByCode(code, fromCache);
            if (dict == null) return new List<MyCreek.Data.Model.Dictionary>();
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public List<MyCreek.Data.Model.Dictionary> GetAllChilds(Guid id, bool fromCache=false)
        {
        ...
        private void addChilds(List<MyCreek.Data.Model.Dictionary> list, Guid id, bool fromCache=false)
        {
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public MyCreek.Data.Model.Dictionary GetByCode(string code, bool fromCache=false)
        {
        ...
        /// <returns></returns>
        private int getParentCount(List<MyCreek.Data.Model.Dictionary> dictList, MyCreek.Data.Model.Dictionary dict)
        {
            int parent = 0;
            MyCreek.Data.Model.Dictionary parentDict = dictList.Find(p => p.ID == dict.ParentID);
        ...
        private string getRadios(List<MyCreek.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getCheckboxs(List<MyCreek.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getOptionsValue(OptionValueField valueField, MyCreek.Data.Model.Dictionary dict)
        {
        ...
        {
            MyCreek.Cache.IO.Opation.Set(cacheKey, GetAll());
        }
        ...
        /// <returns></returns>
        public string GetComboxTableHtmlByID(string id, MyCreek.Platform.Dictionary.OptionValueField valueField, string defaultValue)
        {
            if (!id.IsGuid())
            {
                return "";
