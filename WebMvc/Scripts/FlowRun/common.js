...
function formSubmit(options)
{
    ...
    //{
        ...
        f.action = top.rootdir + "/WorkFlowRun/Execute?" + query + "&isSystemDetermine=" + isSystemDetermine;
        f.submit();
    //}
}
...
function saveData(opation)
{
    ...
    var f = document.forms[0];
    window.setTimeout('', 100);
    f.action = top.rootdir + "/WorkFlowRun/SaveData?" + query + "&opation=" + opation;
    f.submit();
}
...
{
    top.mainDialog.open({ id: 'showprocess', title: '查看处理过程', url:top.rootdir + '/WorkFlowTasks/Detail?' + query, width: 1024, height: 550 });
}

function showFlowDesign()
{
    top.mainDialog.open({ id: 'showflowdesign', title: '查看流程图', url: top.rootdir + '/WorkFlowRun/ShowDesign?' + query, width: 1024, height: 550 });
}

function formPrint()
{
    RoadUI.Core.open(top.rootdir + "/WorkFlowRun/Print?" + query + "&instanceid1=" + $("#instanceid").val() + "&isreadonly=1", 980, 600, "打印表单");
}

function showSubFlow()
{
    top.mainDialog.open({ id: 'showsubflow', title: '查看子流程处理过程', url: top.rootdir + '/WorkFlowTasks/DetailSubFlow?' + query, width: 1024, height: 550 });
}