...
namespace MyCreek.Data.Factory
{
    public class Factory
    {
        /*
        private static string dataType = MyCreek.Utility.Config.DataBaseType;
        private static string cacheKey = MyCreek.Utility.Keys.CacheKeys.ClassInstance_.ToString();
        public static object CreateInstance(string className)
        {
            string cacheKey1 = cacheKey + className;
            string typeName = "MyCreek.Data." + dataType + "." + className;
            object obj = MyCreek.Cache.IO.Opation.Get(cacheKey1);
            if (obj == null)
            {
                Type type = Assembly.Load("MyCreek.Data." + dataType).GetType(typeName, true);
                obj = Activator.CreateInstance(type);
             
                MyCreek.Cache.IO.Opation.Set(cacheKey1, obj);
